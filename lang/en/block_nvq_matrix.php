...
$string['backdateaudit']           = 'Also backdate the audit trail to this date';
$string['backdateaudithelp']       = 'Normally the audit trail always records the real date any change is made, regardless of the date entered above, so it can never be backdated by mistake. Tick this only when entering genuinely historical data (e.g. migrating records from a previous system) - it makes the audit trail record the date entered above instead of today. Available to Assessors, Managers, and site admins only.';
...
$string['migrationmode']         = 'Migration mode (backdated audit trail)';
$string['migrationmode_desc']    = 'Off by default, and permanently non-backdatable for everyone while off - this is the plugin\'s normal, trustworthy state. When switched on, an Assessor or Manager (not IQA/EQA reviewers) entering a grade, grade comment, IQA comment, or final status with a backdated date already causes the resulting audit-trail entry to record that same backdated date, instead of the real moment of entry. Intended only for a temporary window while migrating historical data from a previous platform, where entering genuinely old results would otherwise make every migrated entry look like a change made today. Switch this back off once migration is complete to restore the full guarantee that the audit trail\'s change-timestamp can never be backdated by anyone.';
